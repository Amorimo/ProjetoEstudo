...
const urldb = "mongodb+srv://admin:123senac@projetoestudo.1vloy.mongodb.net/projetoestudo?retryWrites=true&w=majority&appName=ProjetoEstudo"
...
app.get("/", (req, res) => {
    Cliente.find()
        .then((result) => res.status(200).send({ output: "ok", payload: result }))
        .catch((erro) => res.status(500).send({ output: `Erro ao processar dados -> ${erro}` }))
})

app.post("/cadastro", (req, res) => {
    const dados = new Cliente(req.body)
    dados.save()
        .then((result) => res.status(201).send({ output: "Cadastro realizado", payload: result }))
...
app.put("/atualizar/:id", (req, res) => {
    Cliente.findByIdAndUpdate(req.params.id, req.body, { new: true })
        .then((result) => {
            if (!result) return res.status(400).send({ output: "Não foi possível atualizar" })
...
app.delete("/apagar/:id", (req, res) => {
    ...
    }).catch((erro=>console.log(`Erros ao tentar apagar -> ${erro}`)))
})
